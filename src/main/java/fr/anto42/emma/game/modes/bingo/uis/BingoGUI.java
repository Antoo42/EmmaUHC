...
public class BingoGUI {
    ...
    public BingoGUI(BingoModule module, Player player, KInventory previous){
        ...
        back.addCallback((kInventoryRepresentation, itemStack, player1, kInventoryClickContext) -> {
            ...
            else previous.open(player1);
        });
        this.kInventory.setElement(45, back);


        int[] gridSlots = generateGridSlots(module.getBingoConfig().getCartSize());
        List<ItemStack> bingoItems;

        if (player == null) {
            bingoItems = module.getBingoList();
        }else {
            bingoItems = module.getBingoGrid(player.getUniqueId());
        }
