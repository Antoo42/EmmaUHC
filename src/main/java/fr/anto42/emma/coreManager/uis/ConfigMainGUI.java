...
public class ConfigMainGUI {
    ...
    public ConfigMainGUI(UHCGame uhc) {
        ...
        KItem license = new KItem(new ItemCreator(SkullList.BLOCK_COMMANDBLOCK_DEFAULT.getItemStack()).name("§8┃ §6§lUHC").lore("", "§8§l» §fDéveloppeur: §b@Anto42_", "", "§8┃ §fFait avec §damour§f","", "§8§l» §6Cliquez §fpour ouvrir le §cmenu d'administration§f.").get());
        license.addCallback((kInventoryRepresentation, itemStack, player, kInventoryClickContext) -> {
            if (!player.isOp() && !player.hasPermission("uhc.*") && !UHC.getInstance().getUhcGame().getUhcData().getHostPlayer().equals(UHC.getUHCPlayer(player))){
        ...
        starterStuff.addCallback((kInventoryRepresentation, itemStack, player, kInventoryClickContext) -> {
            ...
            if (uhc.getUhcConfig().getStarterStuffConfig().getStartInv().length != 0){
                for (int i = 0; i < uhc.getUhcConfig().getStarterStuffConfig().getStartInv().length; i++) {
                    String string = uhc.getUhcConfig().getStarterStuffConfig().getStartInv()[i];
                    player.getInventory().setItem(i, ItemStackToString.ItemStackFromString(string));
                }
            ...
            msg.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder("§8§l» §6Cliquez§7f pour sauvegarder.").create()));
        ...
        start.addCallback((kInventoryRepresentation, itemStack, player, kInventoryClickContext) -> {
            ...
            if (uhc.isStart()) {
                if (uhc.getGameState() != GameState.WAITING) {
                    player.sendMessage(UHC.getInstance().getPrefix() + " §cVous ne pouvez pas faire ça ! Si vous souhaitez arrêter votre partie, utilisez la commande §a/host stop§c.");
                    SoundUtils.playSoundToPlayer(player, Sound.VILLAGER_NO);
                    return;
                }
                uhc.setStart(false);
