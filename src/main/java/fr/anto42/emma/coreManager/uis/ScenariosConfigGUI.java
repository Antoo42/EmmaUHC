        ...
        if (actualType == ALL) {
            scenarioManager.getInitialScenarioList().stream().filter(scenario -> scenario.getPage() == page).forEach(uhcScenario -> {
                ...
                if (uhcScenario.isConfigurable()){
                    kItem = new KItem(new ItemCreator(uhcScenario.getItemStack()).name("§8┃ §f" + uhcScenario.getName()).lore("", "§8§l» §fStatut: " + (uhcScenario.isActivated() ? "§aactivé" : "§cdésactivé"), "", uhcScenario.getDesc(), "", "§8§l» §fType: §6" + uhcScenario.getScenarioType().getTypeName(), "", "§8§l» §6Clique-gauche §fpour activer.", "§8§l» §6Clique-droit §fpour configurer.").get());
