...
public class SettingsNewWorldGUI {
    ...
    public SettingsNewWorldGUI() {
        ...
        KItem settings = new KItem(new ItemCreator(Material.SAND).name("§8┃ §fmonde test en sable").lore("", "§8┃ §6Configurez§f des éléments tels que", "§8┃ §ale boost de minerais§f, ou de cavernes", "", "§c§o  Cette option n'est pas encore disponible !", "§c§o  Prévue dans une future mise à jour !").get());
        settings.addCallback((kInventory1, item, player, clickContext) ->{
            //new CustomWorldManager().createDesertWorld("test");
        });
