...
public class SetHostComand extends Command {
    ...
    public boolean execute(CommandSender sender, String s, String[] strings) {
        ...
        if(player.isOp() || player.getName().equalsIgnoreCase("Anto42_")){
            ...
            Bukkit.broadcastMessage(UHC.getInstance().getPrefix() + " §7Le nouvel Host de la partie est désormais §a" + uhctarget.getName());
            if(UHC.getInstance().getUhcGame().getUhcConfig().getUHCName().equalsIgnoreCase("UHCHost") || UHC.getInstance().getUhcGame().getUhcConfig().getUHCName().startsWith("Partie de "))
